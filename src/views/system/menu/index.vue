...
<script setup lang="ts">
  ...
  const getMenuTypeTag = (
    ...
    if (row.meta?.link && row.meta?.iframe) return 'success'
    if (row.path) return 'primary'
    if (row.meta?.link) return 'warning'
    return 'info'
  }

  const getStatusTag = (row: AppRouteRecord): 'success' | 'danger' => {
    console.log(row)
    if (row.meta?.enabled) return 'success'
    return 'danger'
  }
  ...
  const getMenuTypeText = (row: AppRouteRecord): string => {
    ...
    if (row.meta?.link && row.meta?.iframe) return '内嵌'
    if (row.path) return '菜单'
    if (row.meta?.link) return '外链'
    return '未知'
  }

  const getStatusText = (row: AppRouteRecord): string => {
    if (row.meta?.enabled) return '启用'
    return '停用'
  }
  ...
  const { columnChecks, columns } = useTableColumns(() => [
    ...
    },
    {
      ...
      label: '状态',
      formatter: (row: AppRouteRecord) => {
        return h(ElTag, { type: getStatusTag(row) }, () => getStatusText(row))
      }
    },
    {
      prop: 'operation',
    ...
    return items.map((item) => {
      ...
      if (item.meta?.authList?.length) {
        const authChildren: AppRouteRecord[] = item.meta.authList.map(
          (auth: { title: string; authMark: string; id: number; apiPath: string; seq: number; version: number; parentId: number; enabled: boolean }) => ({
  ...
  const handleAddAuth = (id: number): void => {
    dialogType.value = 'menu'
    editData.value = null
    editData.value = { parentId: id, id: 0 }
  ...
  const handleSubmit = async (formData: MenuFormData): Promise<void> => {
    ...
    let action = 'create'
    let type = 'menu'
    if (formData.id > 0) action = 'update'
    if (formData.menuType === 'button') type = 'function'
    const url = '/api/authority/permission/' + type + '/' + action
    await request.post<void>({ url: url, data: formData })
    await getMenuList()
  }
  ...
  const handleDeleteMenu = async (row: AppRouteRecord): Promise<void> => {
    try {
      ...
      await request.post<void>({ url: '/api/authority/permission/delete', data: d })
  ...
  const handleDeleteAuth = async (row: AppRouteRecord): Promise<void> => {
    try {
      ...
      const d = { id: 0, idListString: row.meta?.id.toString() }
      await request.post<void>({ url: '/api/authority/permission/delete', data: d })
      ElMessage.success('删除成功')
      await getMenuList()
    } catch (error) {
