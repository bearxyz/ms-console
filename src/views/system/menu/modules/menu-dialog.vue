...
<script setup lang="ts">
  ...
  const rules = reactive<FormRules>({
    ...
    code: [{ required: true, message: '输入权限标识', trigger: 'blur' }]
  })
  ...
  const formItems = computed<FormItem[]>(() => {
    ...
    if (form.menuType === 'menu') {
      return [
        ...
        { label: '权限标识', key: 'code', type: 'input', props: { placeholder: '如：User' } },
        {
          label: createLabelTooltip(
            '组件路径',
      ...
      return [
        ...
        },
        {
          ...
          key: 'apiPath',
          type: 'input',
          props: { placeholder: '如：/system/role' }
        },
        {
          label: '权限排序',
          key: 'seq',
  ...
  const loadFormData = (): void => {
    ...
    if (props.editData.id === 0) isEdit.value = false
