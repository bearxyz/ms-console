...
<script setup lang="ts">
  ...
  const initPermission = async () => {
    const res = await request.get<AppRouteRecord[]>({ url: '/api/authority/permission/list' })
    menuList.value = res
    await nextTick()
    const checkedKeys = getCheckedKeysFromRole()
    console.log('checkedKeys', checkedKeys)
    treeRef.value?.setCheckedKeys(checkedKeys)
  }

  const getCheckedKeysFromRole = () => {
    if (!props.roleData) return []
    const keys: number[] = []
    props.roleData.permissions.forEach((p) => {
      keys.push(p.id)
    })
    return keys
  }
  ...
  const savePermission = async (): Promise<void> => {
    ...
    if (!tree) return
    try {
      const selectedIds = tree.getCheckedKeys().join(',')
      const assign = {
        id: props.roleData?.id,
        idListString: selectedIds
      }
      await request.post<void>({ url: '/api/authority/role/assign_permission', data: assign })
