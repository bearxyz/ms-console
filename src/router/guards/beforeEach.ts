...
async function handleRouteGuard(
  ...
  // 1. 检查登录状态
  if (!handleLoginStatus(to, userStore)) {
    return
  }
...
function handleLoginStatus(
  ...
  try {
    request.get<BaseResponse>({ url: '/public/status' })
    userStore.setLoginStatus(true)
    return true
  } catch (error) {
    console.warn('[RouteGuard] 用户未登录，跳转到登录页')
  }
  ...
  window.location.href = 'http://back.home.local/bff/public/login?redirect=' + url
  return false
}
